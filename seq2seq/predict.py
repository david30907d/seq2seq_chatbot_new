...
model = Seq2SeqModel(FLAGS.rnn_size, FLAGS.num_layers, FLAGS.embedding_size, FLAGS.learning_rate, word2id, mode='decode', use_attention=True, beam_search=True, beam_size=5, max_gradient_norm=5.0)
with tf.Session(config=config) as sess:
    ...
    while sentence:
        print(model.infer(sess, sentence, id2word, isBatch=False))
        print("> ", "")
        sys.stdout.flush()
        sentence = sys.stdin.readline()