...
def gensim2file(filepath, dataset):
    ...
    if dataset == 'dcard':
        ...
        subprocess.call(['python3', 'utils/sogou.py'])
        for new in tqdm.tqdm(json.load(open('utils/sogou.json', 'r'))):
            content = new['content']
...
if __name__ == '__main__':
    ...
    myParser.add_argument('-d', '--dataset', type=str, help="descide which dataset to use")
    myParser.add_argument('-rest', '--removestopword', type=bool, help="remove stopword", default=False)
    args = myParser.parse_args()
    gensim2file(args.filepath, args.dataset)